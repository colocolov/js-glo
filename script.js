...
let allServicePrices = function getAllServicePrices(serv1, serv2) {
  let totalServ = parseInt(serv1) + parseInt(serv2);
  return totalServ;
};

let fullPrice = function getFullPrice(sum, serv1, serv2) {
  let totalPrice = parseInt(sum) + parseInt(serv1) + parseInt(serv2);
  return totalPrice;
};

function getTitle(title) {
  let titleNew = title.trim();
  titleNew.toLowerCase();
  return titleNew;
}
